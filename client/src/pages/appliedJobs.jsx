...
const AppliedJobs = () => {
    ...
    useEffect(() => {
        const fetchJobs = async () => {
            try {
                const res = await axios.get('/jobs/appliedjobs', { withCredentials: true });
    ...
    const bookmarkJobs = async (jobId) => {
        try {
            ...
            alert(error.response?.data?.message || "Could not bookmark job");
        }
    };
    ...
    const withdrawJobs=async(id)=>{
        try {
            ...
            navigate('/')
             ...
             alert(error.response?.data?.message || "Could not withdraw job");   
        }
    }
